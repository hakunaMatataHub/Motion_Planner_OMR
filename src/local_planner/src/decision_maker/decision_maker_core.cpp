...
void DecisionMaker::evaluatePoly(const PolyCoefficient* coeff, const float& t, Waypoint& wp)
{
		...
		wp.x = coeff[0].c1*t*t*t + coeff[0].c2*t*t + coeff[0].c3*t + coeff[0].c4;
		wp.y = coeff[1].c1*t*t*t + coeff[1].c2*t*t + coeff[1].c3*t + coeff[1].c4;
		wp.z = coeff[2].c1*t*t*t + coeff[2].c2*t*t + coeff[2].c3*t + coeff[2].c4;
		wp.yw = coeff[3].c1*t*t*t + coeff[3].c2*t*t + coeff[3].c3*t + coeff[3].c4;
...
void DecisionMaker::globalPath_callback(const planner_msgs::PointsArrayConstPtr& msg)
{
	...
	else if(!bGlobalPlan && (msg->bExecute))
	{
		...
		ROS_INFO( "GLOBAL PATH RECIEVED BY DECISION MAKER");
...
*/			
    		Waypoint wp;
    		prevSampleTime = ros::Time::now();


    		ROS_INFO("Sampling at %f \n", nextSampleTime);
    		stateAtT(nextSampleTime, wp, globalPath);
    		ROS_INFO("State at  T is %f %f %f %f \n", wp.x,wp.y,wp.z, wp.yw);

	    	geometry_msgs::PoseStamped pose_msg;

	    	pose_msg.pose.position.x = wp.x;
	    	pose_msg.pose.position.y = wp.y;
	    	pose_msg.pose.position.z = wp.z;

	    	tf2::Quaternion quat_tf;
   			quat_tf.setRPY( 0, 0,  wp.yw);
   			quat_tf.normalize();   			
	        ...
	        nextSampleTime += (ros::Time::now() - prevSampleTime).toSec();
	        //nextSampleTime += 0.01;
    	...
    	else
    	{
	    	...
	    	pose_msg.pose.position.x = currentPose.x;
	    	pose_msg.pose.position.y = currentPose.y;
	    	pose_msg.pose.position.z = currentPose.z;
